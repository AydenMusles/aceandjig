      ...
      success: function(data){
        ...
        $('#cartform .item .line-total[data-line]').each(function(){
          ...
          if($input.val() != quantity.toString()) {
            ...
            theme.showQuickPopup('You can only have ' + quantity + ' in your cart', $input.closest('.quantity'));
          }
          // plus visibility
...
theme.ProductBlockManager = new function(){
  ...
  _.afterImagesResized = function(container){
    ...
    $('.product-block:not(.fixed-width):not(.onboarding)').each(function(){
...
$(function($){
  ...
  $(document).on('click', '.collection-listing:not(.related-tag-handle) .product-block:not(.collection-block) .product-link', function(){
    ...
    //Only show dropdown if screen is large enough for it to be useful
        if(pageWidth > 767 && !$block.hasClass('regular')) {
          if($slider.length > 0) {
            ...
            if($detailCont.length == 1) {
              if($block.hasClass('expanded')) {
                ...
                // extra payment buttons
                if (Shopify.PaymentButton) {
                  document.addEventListener('shopify:payment_button:loaded.themeQuickBuy', function(){
  ...
  $('#template label').each(function(){
    ...
    $(document).on('focusin focusout', '#pageheader .search-box input[type="text"]', function(e){
      ...
      console.log($('.announcement__inner'));
      $('.announcement__inner').toggleClass('invisible');
    });
